...
| `inec_catalogue_excerpt.json` | `GET https://www.inec.gob.pa/m_2/api/meta/estructura-completa`, **trimmed** to the three variables this connector reads (ids 206, 207 and 232) and flattened out of INEC's theme → category → variables nesting into the list form `_flatten_catalogue()` produces, which is what the connector stores. Every field of the three kept entries is verbatim, `anio_referencia` included — the one field that proves the reference year is read from the catalogue rather than hardcoded, which a test asserts by rewriting it to 2030. | 2026-09-20 |
| `inec_choropleth_232.json` | `GET https://www.inec.gob.pa/m_2/api/data/choropleth?nivel_geografico=Provincia&id_variable=232&anio=2023` — automobiles in circulation per 1,000 inhabitants, byte-for-byte. The **complete** response, not an excerpt, because its eleven rows are what the connector's own completeness check counts: ten provinces plus the source's single `is_total` row, without which nothing would prove the national figure is kept as INEC published it rather than summed from the ten. These three are also the only fixtures in this directory carrying codes below the country level, so they are where the province registry is checked against the source's own coding. | 2026-09-20 |
| `inec_choropleth_206.json` | Same endpoint, `id_variable=206` — residential building counts, byte-for-byte. Recorded as a second variable so the ten-province set is asserted more than once, and because its `unidad_medida` is `unidades` where 232's is `automóviles`: without it nothing would prove the unit comes from the row rather than a constant. | 2026-09-20 |
| `inec_choropleth_207.json` | Same endpoint, `id_variable=207` — non-residential building counts, byte-for-byte. The third and last variable, recorded so the eleven-rows-per-variable check is exercised against every variable the connector reads rather than a sample of them. | 2026-09-20 |
