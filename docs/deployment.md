...
| **Rewrite a dead `http://` redirect `Location` at the proxy.** `uvicorn` runs with `--no-proxy-headers` (deliberate — keeps client-identity trust in exactly one place, REIM's own `X-Forwarded-For` handling), so the ASGI scope it hands the app always reports `scheme=http`. Any 307/308 Starlette's `redirect_slashes` issues therefore built a `Location` starting `http://` — dead on a deployment with no port-80 listener. The `(https_location)` snippet, imported into every `reverse_proxy` block that targets the `api` container, rewrites it to `https://` in place. Fixed at the proxy rather than by re-enabling uvicorn's `ProxyHeadersMiddleware`, which would also re-trust `X-Forwarded-For` and reopen the exact surface `--no-proxy-headers` was set to close. The frontend's own redirects (Caddy's `file_server` directory canonicalization) need no rewrite — they carry no scheme at all, only a relative `Location`. | `test_https_location_snippet_rewrites_http_to_https`. |
