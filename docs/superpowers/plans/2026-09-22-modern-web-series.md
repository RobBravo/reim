...
- Test: `frontend/src/hooks/__tests__/useComparison.test.ts`
...
- [ ] **Step 1: Write API request tests**
...
- [ ] **Step 3: Add exact comparison types and API function**
...
- [ ] **Step 5: Write hook tests and confirm they fail**

Test key separation for differing date ranges and disabled behavior with no selection using the existing `QueryClientProvider` test setup. Run: `cd frontend && npx vitest run src/hooks/__tests__/useComparison.test.ts`. Expected: tests fail because `useComparison` is not defined.

- [ ] **Step 6: Implement `useComparison` and run API/hook tests**
...
Run: `cd frontend && npx vitest run src/lib/__tests__/api.test.ts src/hooks/__tests__/useComparison.test.ts`
...
- [ ] **Step 1: Add the chart dependency and write controls tests**

Add the `lightweight-charts` dependency at the version chosen by the implementer and update the lockfile with the repository's existing package manager. Test accessible labels, multi-country selection, date values and submit output.

- [ ] **Step 2: Run the controls tests and confirm failure**
...
- [ ] **Step 3: Implement responsive accessible controls**
...
- [ ] **Step 5: Write chart lifecycle and content tests**
...
- [ ] **Step 6: Implement the chart wrapper and run its tests**
...
- [ ] **Step 3: Implement URL-driven selection and loading/error states**
...
- [ ] **Step 4: Implement data, comparability and volume rendering**
...
- [ ] **Step 5: Run page tests and adjust until green**
...
Confirm no API, database, deployment or legacy implementation files changed; verify API URL parameters, chart cleanup, URL-restorable selection, no conversion/downsampling, all states, and the unchanged `/legacy/runs` navigation. Report any mismatch before considering release.
