...
def _resolve_key(token: str) -> tuple[str | None, bool]:
    ...
    ``reim key list`` would report every key as never used.
    """
...
Expected: 9 passed. The two reasons a keyed request could wrongly 401 are both already handled in the fixture and the tests, and were verified before this plan was written: the middleware opens its own session, so the fixture writes must be **committed** rather than flushed, and that session must be bound to the **test schema** (`reim_test`) rather than the application engine's `public`. If a keyed test still 401s, report it rather than loosening the assertion.
