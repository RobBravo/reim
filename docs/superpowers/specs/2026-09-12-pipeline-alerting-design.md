...
(a plain `String(32)`, deliberately not `enum_column` — `AlertCondition` already
owns the four values, and a database `CHECK` restating them would be a second
definition of one fact), `pipeline_key` (`String(120)`, matching
...
The trade-off of the plain column: nothing at the database level catches a
typo'd condition value. `AlertCondition` is the only guard.
...
payload (keys sorted, since `build_payload` renders with `json.dumps(...,
sort_keys=True)`; timestamps are `datetime.isoformat()`, which emits a
`+00:00` offset rather than a `Z` suffix):
  ...
  "firing": [
    {
      ...
      "first_notified_at": "2026-09-10T18:00:00+00:00",
      "pipeline_key": "banguat_exchange_rate",
      "severity": "warning",
      "summary": "banguat_exchange_rate has no data newer than 9 days, past its 7-day threshold."
    }
  ],
  ...
  "resolved": [
    {
      ...
      "details": {"last_run_at": "2026-09-11T08:15:00+00:00"},
      "first_notified_at": "2026-09-11T18:00:00+00:00",
      "pipeline_key": "inide_cpi_monthly",
      "summary": "inide_cpi_monthly no longer reports failed_run."
    }
  ],
  "version": 1
}
...
`first_notified_at` on a firing entry is the *original* notification time, not
this run's: a repeat carries the same value across every run of an ongoing
outage, matched from the open row on `(condition, pipeline_key)`, so day 22
still reads as day 1. A genuinely new alert has no open row yet, so it is
first notified now. `details` on a resolved entry is the open row's own —
whatever was sent with the last notification before it cleared — so a
recovery notice can quote what the problem had been without recomputing it
(§3).
...
alert_repeat_hours:     int = 24   (ge=1, le=720)
alert_stuck_run_hours:  int = 6    (ge=1, le=168)
```
...
* `make migrate-down` runs `downgrade -1`, so `downgrade()` must actually work
  rather than stay the autogenerated stub. It is only a `drop_table`: `condition`
  is a plain `String(32)`, not `enum_column`, so there is no PostgreSQL `TYPE`
  and no `CHECK` constraint to drop separately — nothing beyond the table
  itself to reverse. The trade-off is the one noted in §3: no database-level
  guard against a typo'd condition value, because `AlertCondition` already owns
  the four values and a `CHECK` restating them would be a second definition of
  one fact.
